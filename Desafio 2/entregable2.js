...
ok | deleteById(Number): void - Elimina del archivo el objeto con el id buscado.
ok | deleteAll(): void - Elimina todos los objetos presentes en el archivo.
 */
...
class Contenedor {
    ...
    async crearArchivo(productos) {
        ...
        try {
            await fs.promises.writeFile(`${this.desafio}.txt`, JSON.stringify(productos, null, 2), "utf-8");
        } catch (err) {
            console.log(err);
        }
    }
    ...
    async getAll() {
        try {
            const contenido = await fs.promises.readFile(`${this.desafio}.txt`, "utf-8");
    ...
    async deleteAll() {
        try {
            await fs.promises.writeFile(`${this.desafio}.txt`, JSON.stringify([]), "utf-8");
